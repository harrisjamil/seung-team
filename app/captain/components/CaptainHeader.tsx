...
export function CaptainHeader({
  ...
  const navItems = [
    ...
    {
      label: "Communication",
      href: "/captain/communication",
      active: pathname.startsWith("/captain/communication"),
    },
  ];
  ...
  return (
    <>
      <header className="sticky top-0 z-[100] w-full border-b border-slate-200/90 bg-white/95 shadow-[0_1px_0_rgba(15,23,42,0.06)] backdrop-blur-md supports-[backdrop-filter]:bg-white/90 dark:border-slate-700/90 dark:bg-slate-900/95 dark:shadow-[0_1px_0_rgba(0,0,0,0.35)] dark:supports-[backdrop-filter]:bg-slate-900/90">
        <div className="mx-auto flex h-12 w-full max-w-[1920px] items-center gap-2 px-3 sm:h-14 sm:gap-3 sm:px-4 lg:px-6">
          <div className="flex min-w-0 shrink-0 items-center gap-2 sm:gap-3">
            ...
            <span className="hidden shrink-0 rounded-md border border-slate-200 bg-slate-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-slate-600 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300 sm:inline">
              Captain
            </span>
          </div>
      ...
      <MobileNavDrawer open={mobileNavOpen} onClose={() => setMobileNavOpen(false)} title="Captain">
        {navItems.map((item) => (
          <Link
            key={item.href}
            href={item.href}
