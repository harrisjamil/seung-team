 ...
 */
export async function parseDistressMessage(text: string): Promise<DistressStructured> {
...
async function parseWithOpenAI(text: string, apiKey: string): Promise<DistressStructured> {
...
severity (low|medium|high|critical), summary (short), category (string), injuries (number or null), damageEstimate (string or null).`,
  ...
  const severity: DistressStructured["severity"] =
    severityRaw === "low" ||
    severityRaw === "medium" ||
    severityRaw === "high" ||
  ...
  return {
    ...
    injuries:
      typeof obj.injuries === "number"
        ? obj.injuries
        : obj.injuries == null
          ? null
          : Number(obj.injuries) || null,
    damageEstimate:
      obj.damageEstimate == null ? null : String(obj.damageEstimate),
  };
}

export function distressSeverityScore(s: DistressStructured): number {
  const base =
    s.severity === "critical"
      ? 100
      : s.severity === "high"
...
function parseHeuristic(text: string): DistressStructured {
  const lower = text.toLowerCase();
  let severity: DistressStructured["severity"] = "medium";
  if (
    /\b(fire|sinking|mayday|capsiz|abandon|explosion|collision|breach)\b/.test(
      lower,
    )
  ) {
    severity = "critical";
  ...
  if (/\bfire\b/.test(lower)) category = "fire";
  else if (/\bengine\b/.test(lower)) category = "propulsion";
  else if (/\bmedical|\binjur/.test(lower)) category = "medical";
  else if (/\bweather|storm|wave/.test(lower)) category = "weather";
