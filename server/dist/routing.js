...
function snapToGrid(v, origin, step, maxIdx) {
    const idx = Math.round((v - origin) / step);
    return Math.max(0, Math.min(maxIdx, idx));
}
function idxToLatLng(cx, cy, south, west, step) {
    return {
        lat: south + cy * step + step / 2,
        lng: west + cx * step + step / 2,
    };
}
function isOpenCell(fleet, zones, lat, lng) {
    if (!pointInPolygon(fleet.navigableWater, lng, lat))
        return false;
    for (const z of zones) {
        if (pointInRingLngLat(z.ring, lng, lat))
            return false;
    }
    return true;
}
function nearestOpenCell(fleet, zones, south, west, rows, cols, step, target) {
    ...
    const tryCell = (cx, cy) => {
        ...
        if (isOpenCell(fleet, zones, ll.lat, ll.lng))
            return { cx, cy };
        return null;
    };
...
function simplifyPolyline(pts) {
    ...
    for (let i = 1; i < pts.length - 1; i++) {
        ...
        if (Math.abs(shortestAngleDelta(ab, bc)) > 6)
            out.push(b);
    }
    out.push(pts[pts.length - 1]);
    return out;
}
export function computeRoute(fleet, zones, start, goal, stepDeg) {
    ...
    const heuristic = (cx, cy) => nmFromKm(haversineKm(idxToLatLng(cx, cy, south, west, stepDeg), goalLl));
    const open = new Map();
    const closed = new Map();
    const kfun = (cx, cy) => `${cx}:${cy}`;
    ...
    while (open.size > 0) {
        ...
        if (bestNode.cx === gCell.cx && bestNode.cy === gCell.cy) {
            ...
            cells.reverse();
            const gridPts = cells.map((c) => idxToLatLng(c.cx, c.cy, south, west, stepDeg));
            /** Full chain: ship → grid path → precise port */
            const chain = [{ ...start }, ...gridPts.slice(1), { ...goal }];
        ...
        for (const [dx, dy, dn] of NEIGH) {
            ...
            if (!isOpenCell(fleet, zones, cellCenter.lat, cellCenter.lng))
                continue;
            ...
            const mult = dn *
                weatherCostMultiplierAt((fromCenter.lat + cellCenter.lat) / 2, (fromCenter.lng + cellCenter.lng) / 2);
